...
void main() {
    uint bodyIndex = gl_InstanceIndex / TRIANGLES_PER_BODY;
    uint triLocal = gl_InstanceIndex % TRIANGLES_PER_BODY;
    uint triIndex = bodyIndex * TRIANGLES_PER_BODY + triLocal;
    uvec4 tri = triangleIndexBuffer.triIndices[triIndex];
    uint nodeIndex = gl_VertexIndex == 0 ? tri.x : (gl_VertexIndex == 1 ? tri.y : tri.z);
    vec3 worldPos = computedPos[nodeIndex].xyz;
    
    // Extract movement parameters for color calculation from SoA buffers
    vec4 entityMovementParams = movementParamsBuffer.movementParams[bodyIndex];
    float phase = entityMovementParams.z;
    float timeOffset = entityMovementParams.w;
    float entityTime = pc.time + timeOffset;

    vec4 controlParams = controlParamsBuffer.controlParams[bodyIndex];
    ...
    float entityBaseHue = mod(float(bodyIndex) * 0.618034, 1.0); // Golden ratio for good distribution
    
    // Per-entity individualized timing and frequencies - INTENSE VERSION
    float entityFreqMultiplier = 0.3 + mod(float(bodyIndex) * 0.7321, 1.0) * 2.7; // Range: 0.3 to 3.0 (much wider)
    float entityPhaseOffset = mod(float(bodyIndex) * 2.3941, 6.28318530718); // Unique phase offset
    float entityTimeOffset = mod(float(bodyIndex) * 1.4142, 15.0); // Unique time offset (longer spread)
    ...
    float phaseLengthVariation = 0.8 + mod(float(bodyIndex) * 0.8660, 1.0) * 1.7; // Phase length: 0.8-2.5 seconds (much faster)
    ...
    float entityHueShiftAmount = 0.2 + mod(float(bodyIndex) * 0.5257, 1.0) * 0.6; // Shift amount: 20-80% (massive jumps)
    ...
    float entityBrightnessBase = mod(float(bodyIndex) * 0.381966, 1.0);
    float brightnessFreq = 0.4 + mod(float(bodyIndex) * 0.9511, 1.0) * 1.2; // Range: 0.4 to 1.6 (much faster)
    ...
    float entitySaturationBase = mod(float(bodyIndex) * 0.236068, 1.0);
    float saturationFreq = 0.3 + mod(float(bodyIndex) * 0.4472, 1.0) * 1.0; // Range: 0.3 to 1.3 (much faster)
